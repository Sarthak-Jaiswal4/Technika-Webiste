...
function Comepetition() {
  ...
  // --- Effect to Show/Hide Custom Cursor based on Hover State ---
   useEffect(() => {
     ...
     if (isHoveringCard) {
       ...
       gsap.to(cursorElement, { opacity: 1, scale: 1, duration: 0.3, ease: 'power1.out' , opacity:0.8 });
       // Hide default cursor on the container
       containerElement.style.cursor = 'none';
     } else {
       // Animate custom cursor out
       gsap.to(cursorElement, { opacity: 0, scale: 0, duration: 0.3, ease: 'power1.out' });
   ...
   const handleMouseEnterCard = (e) => {
    ...
    gsap.to(e.currentTarget, { scale: 1.2, duration: 0.3, ease: "power1.out", overwrite: "auto" });
  };

  const handleMouseLeaveCard = (e) => {
    setIsHoveringCard(false);
     // Optional: Keep or remove the card scaling effect
    gsap.to(e.currentTarget, { scale: 1, duration: 0.3, ease: "power1.out", overwrite: "auto" });
  };
    ...
    useGSAP(()=>{
        ...
        for(let i=0;i<length-1;i++){
            ...
            if (nameElement) {
              ...
              scrollTimeline.to([nameSplit.lines,infoSplit.lines], {
                opacity: 0,
                y: "-100%",
                // duration: 0.4,
                stagger: 0.05
  ...
  return (
    <div className="w-full h-full bg-gray-900 font-[main-font] main-color ">
      ...
      <div ref={pageref} className="w-full h-screen flex flex-col justify-center items-center px-40 bg-blur gap-40 relative" style={{ backgroundImage: `url('/2cbc84d3-7541-40fa-99a9-69234be52e94.jpg')`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
        {windowopen && windowinfo && (
        ...
        {competitionInfo.map((e,index) => (
          <>
            ...
            >
              <h1 className="text-white text-[4vw] w-[30%] font-black flex flex-col justify-center items-center text-center leading-wider gap-3 sm:leading-[50px]">
                {e.name}
              </h1>
              <Tilt>
                <img
                    ...
                    onClick={(e) =>  {setwindowopen(true);windowopenning(e.currentTarget);SearchImageForWindow(e.currentTarget)} }
              ...
              <h1 className="text-white sm:text-[4vw] w-[30%] font-black flex flex-col justify-center items-center text-center leading-normal sm:leading-[50px]"
              >
                {e.quote}
              </h1>
            </div>
